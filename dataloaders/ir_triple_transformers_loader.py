...
class IrTripleTransformersDatasetReader(DatasetReader):
    ...
    def text_to_instance(self, query_sequence: str, doc_pos_sequence: str, doc_neg_sequence: str) -> Instance:  # type: ignore
        ...
        _truncate = False
        _truncate_up_to = self._max_out_length
        if _truncate_up_to != None:
            _truncate = True
            if self._add_start_end_tokens:
                _truncate_up_to -= 2 # take into account <start> and <end> tokens to be added in the end
        ...
        query_tokenized = self._transformers_tokenizer(' '.join(query_pre_tokenized),
                                                       truncation = _truncate,
                                                       add_special_tokens = self._add_special_tokens,
                                                       max_length = _truncate_up_to)["input_ids"]
        ...
        # ADAPT ADDITNG START AND END
        if self._add_start_end_tokens:
            query_tokenized.insert(0, self._transformers_tokenizer.vocab["[unused11]"])
            query_tokenized.append(self._transformers_tokenizer.vocab["[unused12]"])

            doc_pos_tokenized.insert(0, self._transformers_tokenizer.vocab["[unused11]"])
            doc_pos_tokenized.append(self._transformers_tokenizer.vocab["[unused12]"])

            doc_neg_tokenized.insert(0, self._transformers_tokenizer.vocab["[unused11]"])
            doc_neg_tokenized.append(self._transformers_tokenizer.vocab["[unused12]"])
