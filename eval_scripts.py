...
def save_sorted_dist_results(results,file,until_rank=-1):

    num_models = len(random.choice(list(results.values())))
    print (num_models)
    val_dist_file = open(file,'w')

    for qid in results.keys():
        query_data = list(results[qid].items())
        query_data.sort(key=lambda x: sum(x[1])/num_models, reverse=True)
        # sort the results per query based on the output, in the case of bayesian, take the mean
        for rank_i, (docid, score) in enumerate(query_data):
            dist_score_str = ' '.join(['%.6f'%m_score for m_score in score])
            val_dist_file.write("%s Q0 %s %d %s uncertainty\n" % (str(qid), str(docid), rank_i + 1, dist_score_str))
            if until_rank > -1 and rank_i == until_rank + 1:
                break

def save_sorted_results(results, file, until_rank=-1,mean=False):
