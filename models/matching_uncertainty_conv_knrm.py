...
class Uncertainty_Conv_KNRM(Model):
    '''
    ...
    def __init__(self,
        ...
        self._stochastic_layer = torch.nn.Linear(n_kernels * 9,n_kernels * 9,bias=True)
        self._output_projection_layer = torch.nn.Linear(n_kernels * 9, 1,bias=False)
    ...
    def forward(self, query: Dict[str, torch.Tensor], document: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for _ in range(num_iters):
            all_grams_num = self.cd1(all_grams,torch.nn.Sequential(self._stochastic_layer, self.m))
            all_grams_num = self.cd2(all_grams_num,torch.nn.Sequential(self._output_projection_layer))
            rels.append(all_grams_num)
        if self.training:
            rels = rels[0]
        else:
