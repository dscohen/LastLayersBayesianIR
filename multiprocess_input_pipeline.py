...
def multiprocess_training_loader(process_number: int, _config, _queue: mp.Queue, _wait_for_exit: mp.Event, _local_file, vocabulary):
    ...
    elif _config["token_embedder_type"] == "bert":
        _bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')

        model_is_discobert = (_config["model"]=="discbert")

        out_tokens_number_limit = None
        if not model_is_discobert:
            out_tokens_number_limit = 512 #standard input limitation for BERT

        _triple_loader  = IrTripleTransformersDatasetReader(lazy=True, transformers_tokenizer = _bert_tokenizer,
                                                            add_special_tokens = False,
                                                            max_doc_length = _config["max_doc_length"],
                                                            max_query_length = _config["max_query_length"],
                                                            add_start_end_tokens = not model_is_discobert,
                                                            max_out_length_in_tokens = out_tokens_number_limit)
        _iterator = BucketIterator(batch_size=int(_config["batch_size_train"]),
...
def multiprocess_validation_loader(process_number: int, _config, _queue: mp.Queue, _wait_for_exit: mp.Event, _local_file, vocabulary):
    ...
    if _config["token_embedder_type"] in ["embedding", "random", "elmo"]:
        ...
        # LOADING BERT-TOKENIZER FOR THE SECOND TIME, NOT GOOD #
        _bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        ...
        out_tokens_number_limit = None
        if not model_is_discobert:
            out_tokens_number_limit = 512

        _tuple_loader  = IrTupleTransformersDatasetReader(lazy=True, transformers_tokenizer=_bert_tokenizer,
                                                            add_special_tokens=False,
                                                            max_doc_length=_config["max_doc_length"],
                                                            max_query_length=_config["max_query_length"],
                                                            add_start_end_tokens= not model_is_discobert,
